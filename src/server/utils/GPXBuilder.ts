        ...
        booking.days.forEach(day => {
            day.events.forEach(event => {
                switch (event.product.type) {
                    case undefined/*EventType.DRIVING*/: // FIXME No tiene que ser asi, deberiamos tener un type en cada evento de cada dia, para distinguir de que se trata. Ponerlo en el insert de un booking.
                        let route: RouteDTO = event.product as RouteDTO
                        ...
                        // Route Origin
                        order += 1
                        gpxContent = gpxContent.concat(this.addWaypoint(route.origin.name, route.origin.geo.point.lat, route.origin.geo.point.lng, order))
                        ...
                        // Route Destination
                        order += 1
                        gpxContent = gpxContent.concat(this.addWaypoint(route.destination.name, route.destination.geo.point.lat, route.destination.geo.point.lng, order))
                        routes = routes.concat(this.addRoutePoint(route.destination.name, route.destination.geo.point.lat, route.destination.geo.point.lng, order))
                    ...
                    default:
                        order += 1
                        let place: PlaceDTO = event.product as PlaceDTO
