...
export class CompanyService {
  ...
  getAll (paginationMetadata?: PaginationOptionsInterface, withoutMetadata?: boolean): Observable<any> {
    ...
    return this.http.get('/api/companies' + queryParams);
  }

  signin(params): Observable<any> {
    return this.http.post(`/api/companies/signin`, params);
  }

  upsert (params): Observable<any> {
    if (params.id && params.id !== '')
      return this.http.patch(`/api/companies/${params.id}`, params.body);
    else
      return this.http.post('/api/companies', params.body);
  }
